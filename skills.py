...
def fetch_news():
    ...
    response = requests.get('https://www.youtube.com/watch?v=aE6nPVKuzlM')
...
def suggest_movie():
    ...
    response = requests.get('https://kinogo.no/')
    soup = BeautifulSoup(response.text, 'html.parser')
...
def respond_to_command(command1) -> object:
    """Respond to the given command."""
    func = data_set.get(command1.lower())
    if func:
        result = func()
